...
const calculateMatrices = (aspect: number, transformation: Transformation):
  {modelViewMatrix: Float32Array, normalMatrix: Float32Array} => {
...
const bindBuffers = (glContext: GLContext): void => {
  ...
  if (textureAddressBuffer.numItems > 0) {
    ...
    gl.vertexAttribPointer(
      attributeLocations.textureAddress,
      numTexAddComponents,
      texAddType,
      normalizeTexAdd,
      texAddStride,
