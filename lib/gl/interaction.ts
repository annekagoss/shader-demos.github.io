...
import { DEFAULT_ROTATION_SPEED } from './defaults';

import { interpolateVectors, clamp, addVectors, degreesToRadians } from './helpers';
...
export const applyInteraction = (
  { transformation, interaction }: {transformation: Transformation, interaction: Interaction}
  ): {newTransformation: Transformation, newInteraction: Interaction} => {
  const { accelerateTimer, decelerateTimer } = interaction;

  const newVelocity = calculateVelocity(interaction);

  const newAccelerateTimer =
    accelerateTimer < 1 ? clamp(accelerateTimer + INTERACTION_SETTINGS.friction, 0, 1) : accelerateTimer;

  const newDecelerateTimer =
    decelerateTimer < 1 ? clamp(decelerateTimer + INTERACTION_SETTINGS.friction, 0, 1) : decelerateTimer;
...
const normalizeScreenCoordinates = (
