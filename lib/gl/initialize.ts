...
export function assignStaticUniforms(
	...
	if (mesh.materials) {
		Object.keys(mesh.materials).forEach((name, i) => {
			if (i <= MAX_SUPPORTED_MATERIAL_TEXTURES) {
				...
				if (mat.textures && mat.textures.diffuseMap) {
					...
					gl.uniform1i(
						programInfo.uniformLocations[`uSampler${i}`],
						i
					);
					gl.uniform1f(
						programInfo.uniformLocations[`uHasTexture`],
						1
					);
				}
		...
		gl.uniform3fv(programInfo.uniformLocations[`uDiffuseColor0`], [
			1,
			1,
			1
		]);
		gl.uniform3fv(programInfo.uniformLocations[`uEmissiveColor0`], [
			0,
			0,
			0
		]);
		gl.uniform3fv(programInfo.uniformLocations[`uSpecularColor0`], [
			1,
			1,
			1
		]);
		...
		gl.uniform3fv(programInfo.uniformLocations[`uDiffuseColor1`], [
			1,
			1,
			1
		]);
		gl.uniform3fv(programInfo.uniformLocations[`uEmissiveColor1`], [
			0,
			0,
			0
		]);
		gl.uniform3fv(programInfo.uniformLocations[`uSpecularColor1`], [
			1,
			1,
			1
		]);
		...
		gl.uniform3fv(programInfo.uniformLocations[`uDiffuseColor2`], [
			1,
			1,
			1
		]);
		gl.uniform3fv(programInfo.uniformLocations[`uEmissiveColor2`], [
			0,
			0,
			0
		]);
		gl.uniform3fv(programInfo.uniformLocations[`uSpecularColor2`], [
			1,
			1,
			1
		]);
	...
	gl.uniform3fv(
		programInfo.uniformLocations.uLeftLightColor,
		colors.leftLight
	);
	gl.uniform3fv(
